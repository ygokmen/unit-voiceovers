...
[{
	...
	if (_distance > 200) then {
		...
		[_unit,selectRandom (missionNamespace getVariable (format["UVO_targDownLo_%1",_nationality]))] call FUNC(globalSay3D);
	} else {
		// Dramatic kill confirm at close range
		[_unit,selectRandom (missionNamespace getVariable (format["UVO_targDownHi_%1",_nationality]))] call FUNC(globalSay3D);
	};
