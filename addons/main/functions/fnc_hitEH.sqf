...
if ((side group _unit) getFriend (side group _instigator) >= 0.6) then {
	[_unit,selectRandom (missionNamespace getVariable format ["UVO_friendlyFire_%1",_nationality])] call FUNC(globalSay3D);
} else {
	[_unit,selectRandom (missionNamespace getVariable format ["UVO_wounded_%1",_nationality])] call FUNC(globalSay3D);
};
