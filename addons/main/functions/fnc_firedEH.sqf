...
if (_weapon in ["Throw","Put"]) then {
	private _type = switch true do {
		case (_magazine in UVO_fragTypes) : {
			[{_this call FUNC(thrownFrag);},[_projectile,side group _unit],0.5] call CBA_fnc_waitAndExecute;
			"frag"
		};
	...
	[_unit,selectRandom (missionNamespace getVariable format ["UVO_%1_%2",_type,_unit getVariable "UVO_nationality"])] call FUNC(globalSay3D);
} else {
	...
	if (isPlayer _unit)	then {
		...
		[{
			...
			if (inputAction "reloadMagazine" > 0) exitwith {
				...
				[_unit,selectRandom (missionNamespace getVariable format ["UVO_reloading_%1",_nationality])] call FUNC(globalSay3D);
			};
