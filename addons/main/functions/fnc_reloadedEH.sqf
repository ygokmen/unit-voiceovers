...
if (currentweapon _unit != secondaryweapon _unit) then {
	...
	[_unit,selectRandom (missionNamespace getVariable format ["UVO_ammoLow_%1",_nationality])] call FUNC(globalSay3D);
} else {
	...
	[_friendlyUnit,selectRandom (missionNamespace getVariable format ["UVO_cover_%1",_nationality])] call FUNC(globalSay3D);
};
