...
goko_fnc_Infragfx = 
{
	...
	{ 
		...
		_findnearby = _frag nearEntities [["B_Soldier_base_F", "O_Soldier_base_F"], 16];
...
goko_fnc_Inreloadedfx = 
{
	...
	if (_countMagz != 0) then 
	{
		_null = _this spawn 
		{
			...
			{
				_indisels = _duder nearEntities [["I_Soldier_base_F"], 30]; 
	...
	} else
	{
		...
		{
			_indisels = _unit nearEntities [["I_Soldier_base_F"], 30]; 
