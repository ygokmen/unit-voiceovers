...
goko_fnc_Indmgwoundsys = 
{
	...
	if (!alive _victim) then 
	{
		...
		if (side _shooter == west) then 
		{
			...
			if (_meters > 100) then 
			{
				[_shooter, [_btargetdown, goko_vo_soundsdiameter, goko_vo_soundsamplepitch]] remoteExec ["say3D", 0];
			} else 
			{
				[_shooter, [_suppbtargetdown, goko_vo_soundsdiameter, goko_vo_soundsamplepitch]] remoteExec ["say3D", 0];
			};
		};
	};
};
