/*
	goko unit voice-overs addon eb/east base kill confirm. Called inside switch/killedEH of Unit
	author: gökmen
	website: https://github.com/the0utsider
	description: kill confirm, east. Code passes on 'instigator' via remote exec, gets global from here
*/
params ["_unit", "_instigator"];
/*	look for East side base class. */
_searchFriendly = _instigator nearEntities [["SoldierEB"], 50];
...
_null = _this spawn {
	...
	if ((_localActor distance _victim) > 200) then
	{
		_soundSample = selectRandom [
			"otarget01", "otarget02", "otarget03", "otarget04", "otarget05", 
			"otarget06", "otarget07", "otarget08", "otarget09", "otarget10"
		];
	...
	{
		_soundSample = selectRandom [
			"osupptarget01", "osupptarget02", "osupptarget03", "osupptarget04", "osupptarget05", 
